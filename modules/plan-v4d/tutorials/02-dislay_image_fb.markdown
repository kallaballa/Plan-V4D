...
Here is the complete source code for this example. You can find it in `samples/display_image_fb.cpp`.
...
We define a `Plan` named `DisplayImageFB`. It holds two `UMat` objects: `image_` for the original image and `converted_` for the version that's been resized and color-converted for display.
...
- **`cvtColor(...)`**: We convert the image's color format from the default BGR (used by `imread`) to BGRA, which is what the framebuffer expects.
...
```
- **`ConfigFlags::DISPLAY_MODE`**: This new flag is passed to `V4D::init`. It tells V4D that we intend to use it primarily for direct framebuffer rendering, which can enable certain optimizations.
