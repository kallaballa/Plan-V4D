...
struct values_equal : std::bool_constant<V1 == V2> { using type = T; };

template<typename T>
struct default_type : std::true_type { using type = T; };

template <typename, typename = void>
struct has_call_operator_t : std::false_type {};
template <typename T>
struct has_call_operator_t<T, std::void_t<decltype(&T::operator())>>
    : std::is_same<std::true_type, std::true_type> {};

template <typename, typename = void>
struct has_return_type_t : std::false_type {};
template <typename T>
struct has_return_type_t<T, std::void_t<decltype(&T::return_type)>>
    : std::is_same<std::true_type, std::true_type> {};
...
template <typename, typename = void>
struct element_t : std::false_type { using type = std::false_type; };
template <typename Tptr>
struct element_t<Tptr, std::void_t<decltype(&Tptr::get)>>
    : std::is_same<std::true_type, std::true_type> {
    using type = std::remove_pointer_t<typename Tptr::element_type>;
};

template <typename, typename = void>
struct return_t : std::false_type { using type = std::false_type; };
template <typename Tfn>
struct return_t<Tfn, std::void_t<decltype(&Tfn::operator())>>
    : std::is_same<std::true_type, std::true_type> {
...
auto sub_tuple(tuple&& t) {
    return sub_tuple<offset, len, tuple>(
        std::forward<tuple>(t), std::make_index_sequence<len>());
}
...
struct edgefun_t {
    ...
    static_assert(!std::is_same<return_type_t, std::false_type>::value,
                  "Invalid callable passed");
...
class PLAN_EXPORTS SharedVariables {
    ...
    typedef typename std::map<size_t,
        std::pair<size_t, Ptr<std::mutex>>>::iterator SharedVarsIter;
        ...
        for (const auto& p : sharedVars_) {
            ...
            if (varOffset > registeredOffset) {
                ...
                if (varOffset < (registeredOffset + registeredSize))
                    return {static_cast<size_t>(registeredOffset),
                            static_cast<size_t>(registeredSize)};
            }
        }
        return {0, 0};
    }
...
public:
    ...
    static bool isPlanMember(Tplan& plan, Tvar& var) {
        ...
        PLAN_Assert((parentOffset == 0 && parentActualSize == 0) ||
    ...
    void makeSharedVar(const T& candidate) {
        {
            ...
            PLAN_Assert(safeVars_.find(reinterpret_cast<size_t>(&candidate)) ==
                        safeVars_.end());
        }
        ...
        if (parent.first != 0) {
            ...
            sharedVars_.insert({reinterpret_cast<size_t>(&candidate),
                std::make_pair(sizeof(T), makePtr<std::mutex>())});
        }
    }
    ...
    bool checkShared(Tplan& plan, const T& candidate) {
        ...
        if (sharedVars_.find(reinterpret_cast<size_t>(&candidate)) != sharedVars_.end())
            return true;
        if (!isPlanMember(plan, candidate)) {
            auto parent = findSharedParent(candidate);
            if (parent.first != 0) {
                auto it = sharedVars_.find(parent.first);
                PLAN_Assert(it != sharedVars_.end());
    ...
    void registerSafe(const T& safe) {
        std::lock_guard<std::mutex> guard(safeVarsMtx_);
        if (safeVars_.find(reinterpret_cast<size_t>(&safe)) == safeVars_.end())
    ...
    void safe_copy(const T& from, T& to) {
        ...
        if (mtx == nullptr)
            throw std::runtime_error(
                "Internal error: Trying to safe copy non-shared variables.");
        std::lock_guard<std::mutex> guard(*mtx);
        to = copy(from);
    }

    template<typename T>
    static void copy(const T& from, T& to) { to = copy_construct(from); }

    template<typename T>
    static T safe_copy(const T& from) { T to; safe_copy(from, to); return to; }

    template<typename T>
    static T copy(const T& from) { T to; copy(from, to); return to; }

    template<typename T>
    static T copy_construct(const T& t) { return t; }
    ...
    template<typename T> void lock(const T& shared)   { getMutexPtr(shared)->lock(); }
    template<typename T> void unlock(const T& shared) { getMutexPtr(shared)->unlock(); }
    template<typename T> bool tryLock(const T& shared){ return getMutexPtr(shared)->try_lock(); }
};
...
public:
    struct Keys {
        enum Enum {
            FRAME_CNT, CAPTURE_CNT, FPS_CNT, RUN_CNT, START_TIME, FPS,
            WORKERS_READY, WORKERS_STARTED, LOCKING, DISPLAY_READY,
            LOCK_CONTENTION_CNT, LOCK_CONTENTION_RATE, LCR_CNT,
            SHOW_GUI, TIME_TRACKER
        };
    };
...
private:
    ...
    PLAN_EXPORTS static std::map<string,
        std::pair<std::thread::id, Ptr<std::mutex>>> nodeLockMap_;
    PLAN_EXPORTS static SharedVariables sharedVars_;

    PLAN_EXPORTS static Ptr<std::mutex> getNodeLockInternal(
        const string& name, const bool owned = true);
...
public:
    ...
    template <typename V>
    static const auto& get(Keys::Enum k) { return map_.get<V>(k); }
...
public:
    struct Keys { enum Enum { WORKER_INDEX }; };

private:
    PLAN_EXPORTS static thread_local ThreadSafeAnyMap<Keys::Enum> map_;

public:
    static void init_keys() { create<false, size_t>(Keys::WORKER_INDEX, 0); }
...
PLAN_EXPORTS inline uint64_t get_epoch_nanos() {
