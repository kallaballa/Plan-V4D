...
private:
	PLAN_EXPORTS std::vector<Value> properties_;
    template<typename V>
    ...
    void check_write(K key) {
    	PLAN_Assert(properties_.size() > key);
...
public:
    ...
    void set(K key, const V& value, bool fire = true) {
    	...
    	if(!p)
            throw std::runtime_error(std::string("Type mismatch for key: ") + std::to_string(int(key)) + ". Expected: " + detail::demangle(properties_[key].type().name()) + ", Got: " + detail::type_name<V>() + ".");
    	V oldVal = *p;
    	*p = value;
