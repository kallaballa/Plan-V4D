...
private:
    ...
    static constexpr bool hasCopyBacks_ = (
        (std::remove_reference_t<Ts>::copy_t::value && !std::remove_reference_t<Ts>::read_t::value) && ...);
...
cv::Ptr<Transaction> make_transaction(F f, Args... args) {
    return cv::Ptr<Transaction>(new detail::TransactionImpl<TcountContention, std::decay_t<F>, std::remove_cv_t<Args>...>(f, args...));
}
