...
#ifndef OPENCV_PLAN_TRANSACTION_HPP_
#define OPENCV_PLAN_TRANSACTION_HPP_
...
enum Operators {
    CONSTRUCT_, ASSIGN_, ADD_, SUB_, MUL_, DIV_, MOD_,
    INCL_, INCR_, DECL_, DECR_, AND_, OR_,
    EQ_, NEQ_, LT_, GT_, LE_, GE_, NOT_,
    XOR_, BAND_, BOR_, SHL_, SHR_, IF_, IDX_,
};
...
static auto make_operator_func(Tfirst, Args ...) {
    ...
    if constexpr (Top == Operators::CONSTRUCT_) {
        ...
        return [](typename Tfirst::ref_t l, typename Args::ref_t ... r)
            -> decltype(l = (r + ...)) {
            auto tup = std::forward_as_tuple(r...);
            return l = std::get<0>(tup);
        };
    } else if constexpr (Top == Operators::ADD_) {
        return [](typename Tfirst::ref_t f, typename Args::ref_t ... v)
            -> decltype(f + (v + ...)) { return f + (v + ...); };
        ...
        return [](typename Tfirst::ref_t f, typename Args::ref_t ... v) {
            auto tup = std::forward_as_tuple(v...);
            return f ? std::get<0>(tup) : std::get<1>(tup);
        };
    } else if constexpr (Top == Operators::IDX_) {
        static_assert(binary, "Invalid number of arguments to IDX");
        return [](typename Tfirst::ref_t f, typename Args::ref_t ... v)
            -> decltype(f[std::get<0>(std::forward_as_tuple(v...))]) {
            return f[std::get<0>(std::forward_as_tuple(v...))];
        };
    } else {
        static_assert(true, "Internal Error. Unknown operator value");
        return [](){};
    }
}
...
struct Operation {
    ...
    static auto op(Ttuple operands, std::index_sequence<idx...>) {
        ...
        return std::get<0>(operands).plan()->template OP<TopEnum>(
            std::get<idx>(operands)...);
    }
    template<Operators TopEnum, typename Ttuple>
    static auto op(Ttuple operands) {
        return op<TopEnum>(operands,
...
template<typename T, bool Tcopy, bool Tread, bool Tshared = false,
         typename Tbase = std::false_type, bool TbyValue = false>
class Edge : public EdgeBase {
    template <typename, typename = void>
    struct has_deref_t : std::false_type {};
    template <typename Tval>
    struct has_deref_t<Tval, std::void_t<decltype(&Tval::operator*)>>
        : std::is_same<std::true_type, std::true_type> {};

    template <typename, typename = void>
    struct has_arrow_t : std::false_type {};
    template <typename Tval>
    struct has_arrow_t<Tval, std::void_t<decltype(&Tval::operator->)>>
        : std::is_same<std::true_type, std::true_type> {};

    template <typename, typename = void>
    struct has_get_t : std::false_type {};
    template <typename Tval>
    struct has_get_t<Tval, std::void_t<decltype(&Tval::get)>>
        : std::is_same<std::true_type, std::true_type> {};
...
public:
    ...
    using value_type_t = typename std::disjunction<
        values_equal<read_t::value, true,
            const typename std::remove_pointer<typename std::remove_extent<T>::type>::type>,
    ...
    using func_t     = values_equal<!std::is_same<typename return_t<value_type_t>::type, std::false_type>::value, true, std::true_type>;
    ...
    using issmart_t   = typename std::conjunction<
        has_deref_t<value_type_t>, has_arrow_t<value_type_t>, has_get_t<value_type_t>,
...
private:
    ...
    static_assert(shared_t::value || !(copy_t::value && !read_t::value),
        "Internal error: Trying to form copy-write edge on a non-shared variable.");
    static_assert(!lockie_t::value || !copy_t::value,
        "Internal error: Trying to form a copy edge on a to be locked variable.");
    ...
    >::type;
    using internal_base_ptr_t = typename std::disjunction<default_type<internal_base_t*>>::type;
...
public:
    ...
    void set(pass_t t) {
        if constexpr (issmart_t::value && byvalue_t::value)
            holder_ = std::shared_ptr<base_t>(&t, [](base_t*){});
    ...
    std::mutex& getMutex() {
        static_assert(lockie_t::value,
            "Internal Error: Trying to get mutex from a non-lockie edge");
        return *GlobalState::shared_vars().getMutexPtr(*ptr(), true);
    }

    bool tryLock() { return getMutex().try_lock(); }
    bool unlock()  { getMutex().unlock(); return true; }
...
struct BranchType {
    enum Enum { NONE = 0, SINGLE = 1, PARALLEL = 2, ONCE = 4, PARALLEL_ONCE = 8 };
};
...
auto make_unique_lock_ptr_tuple(Ttuple& t, std::index_sequence<Tidx...>) {
    return std::make_tuple(
        std::unique_lock<std::mutex>(std::get<Tidx>(t).getMutex(), std::defer_lock)...);
}
...
public:
    ...
    TransactionImpl(FwdF func, FwdTs... fwdArgs)
        : f(func), args_{std::make_tuple(fwdArgs...)}, ran_(false) {}

    virtual ~TransactionImpl() override {}
    virtual bool ran() override { return ran_; }
    ...
    void performImpl(_Fn&& __f, _Tuple&& __t, std::index_sequence<_Idx...> seq) {
        if constexpr (hasLockies_) {
            ...
            std::invoke(std::forward<_Fn>(__f), std::get<_Idx>(std::forward<_Tuple>(__t)).ref()...);
    ...
    constexpr decltype(auto) performImplRet(_Fn&& __f, _Tuple&& __t, std::index_sequence<_Idx...> seq) {
        ...
        if constexpr (hasLockies_) {
            ...
            auto lockHolders = make_unique_lock_ptr_tuple<false>(lockies, std::make_index_sequence<lksz>());
            perform_lock_from_tuple(lockHolders, std::make_index_sequence<lksz>());
            res = std::invoke(std::forward<_Fn>(__f), std::get<_Idx>(std::forward<_Tuple>(__t)).ref()...);
            if constexpr (hasCopyBacks_)
                (std::get<_Idx>(std::forward<_Tuple>(__t)).copyBack(), ...);
        } else {
            res = std::invoke(std::forward<_Fn>(__f), std::get<_Idx>(std::forward<_Tuple>(__t)).ref()...);
    ...
    virtual void perform() override {
        if constexpr (!predicate_t::value) {
            ...
            performImpl(std::forward<F>(f), std::forward<decltype(args_)>(args_), _Indices{});
            ran_ = true;
        }
    }
    ...
    virtual bool performPredicate() override {
        if constexpr (predicate_t::value) {
            using _Indices = std::make_index_sequence<std::tuple_size<decltype(args_)>::value>;
            bool res = performImplRet(std::forward<F>(f), std::forward<decltype(args_)>(args_), _Indices{});
            ran_ = true;
            return res;
        }
        return false;
    ...
    virtual bool isPredicate() override  { return predicate_t::value; }
    virtual bool hasLockies() override   { return hasLockies_; }
    virtual bool hasCopyBacks() override { return hasCopyBacks_; }
};
...
#endif // OPENCV_PLAN_TRANSACTION_HPP_
