...
const cv::utils::logging::LogTag plan_cf_tag("PlanFlow", cv::utils::logging::LogLevel::LOG_LEVEL_INFO);
const cv::utils::logging::LogTag plan_tag("Plan", cv::utils::logging::LogLevel::LOG_LEVEL_INFO);
...
class CV_EXPORTS Plan {
    ...
    void pf(const size_t& depth, const BranchState& current, const cv::Ptr<Node> n) {
        if(PlanDebugFlags::PRINT_CONTROL_FLOW & debugFlags_) {
            ...
            CV_LOG_INFO(&plan_cf_tag, formattedName);
            CV_LOG_INFO(&plan_cf_tag, formattedInfo);
        }
    }
    ...
    static void runLoop(cv::Ptr<Plan> plan, std::function<void()> runGraph) {
...
public:
    ...
    cv::Ptr<Plan> set(std::tuple<GlobalState::Keys::Enum,Tedge>&& values, std::index_sequence<idx...>) {
    ...
    template <typename Tedge>
    cv::Ptr<Plan> set(std::tuple<GlobalState::Keys::Enum,Tedge>&& values) {
    ...
    static void run(int32_t workers, Args&& ... args) {
        ...
        try {
            ...
            CV_Error_(cv::Error::StsError, ("Main plan run: %s", ex.what()));
        }

        CV_LOG_DEBUG(&plan_tag, "Main plan run finished: " << LocalState::get<size_t>(LocalState::Keys::WORKER_INDEX));
