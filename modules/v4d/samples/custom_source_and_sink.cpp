...
public:
	void find(const cv::UMat &bgra) {
		...
		if (lastColor_ != pix && isPureColor) {
			cv::Vec4b binarized = convert_pix<-1, Scalar, cv::Vec4b, true>(pix,
					1.0 / 255.0);
...
int main() {
	...
	cv::Ptr<Sink> videoSink = Sink::make(runtime,
			"custom_source_and_sink.mkv", 10, viewport.size());
	...
	cv::Ptr < Sink > sink = new Sink(
			[videoSink](const uint64_t &seq, const cv::UMat &frame) {
