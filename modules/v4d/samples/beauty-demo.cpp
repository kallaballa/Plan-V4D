...
struct FaceFeatures {
	...
	FaceFeatures(const cv::Rect &faceRect, const vector<cv::Point2f> &shapes,
			const double &scale) :
			faceRect_(
					cv::Rect(faceRect.x * scale, faceRect.y * scale,
							faceRect.width * scale, faceRect.height * scale)), scale_(
					scale) {
		...
		allPoints_.insert(allPoints_.begin(), bottomNose.begin(),
				bottomNose.end());
		allPoints_.insert(allPoints_.begin(), leftEyebrow.begin(),
				leftEyebrow.end());
		allPoints_.insert(allPoints_.begin(), rightEyebrow.begin(),
				rightEyebrow.end());
		...
		allPoints_.insert(allPoints_.begin(), outerLips.begin(),
				outerLips.end());
		allPoints_.insert(allPoints_.begin(), insideLips.begin(),
				insideLips.end());

		allFeatures_ = { chin, topNose, bottomNose, leftEyebrow, rightEyebrow,
				leftEye, rightEye, outerLips, insideLips };
	}
	...
	void drawFaceOval() const {
		...
		ellipse(rotRect.center.x, rotRect.center.y * 0.875,
				rotRect.size.width / 2, rotRect.size.height / 1.75);
...
public:
	FaceFeatureExtractor(const cv::Size &inputSize, const float &inputScale) :
	...
	bool extract(const cv::UMat &inputFrame, FaceFeatures &outputFeatures) {
		...
		if (!faces_.empty())
			faceRects_.push_back(
					cv::Rect(int(faces_.at<float>(0, 0)),
							int(faces_.at<float>(0, 1)),
							int(faces_.at<float>(0, 2)),
							int(faces_.at<float>(0, 3))));

		//find landmarks if faces have been detected
		bool found = !faceRects_.empty()
				&& facemark_->fit(inputFrame, faceRects_, shapes_);
		if (found)
...
static void adjust_saturation(const cv::UMat &srcBGR, cv::UMat &dstBGR,
		float factor, std::vector<cv::UMat> &channel) {
	cv::UMat tmp;
...
private:
	...
	static void compose_result(Frames &frames, const Params &params) {
		if (params.sideBySide_) {
			...
			cv::Rect rrect(frames.result_.cols / 2.0, 0, rhalf.cols,
					rhalf.rows);
	...
	constexpr static auto UMAT_COPY_TO_ = _OLMC_(void, cv::UMat,
			&cv::UMat::copyTo, cv::OutputArray);
public:
	BeautyDemoPlan() {
		//construct sub-plans only in the constructor
		prepareFeatureMasksPlan_ = _sub < FaceFeatureMasksPlan
				> (this, features_, frames_);
...
public:
	FaceFeatureMasksPlan(const FaceFeatures &inputFeatures,
			BeautyDemoPlan::Frames &inputOutputFrames) :
	...
	void infer() override {
		...
		->fb(cv::cvtColor, RW(inputOutputFrames_.faceOval_),
				V(cv::COLOR_BGRA2GRAY), V(0), V(cv::ALGO_HINT_DEFAULT))->nvg(
				&FaceFeatures::drawEyesAndLipsMask, RS(inputFeatures_))->fb(
				cv::cvtColor, RW(inputOutputFrames_.eyesAndLipsMaskGrey_),
...
class BeautyFilterPlan: public V4DPlan {
	...
	cv::Ptr<cv::detail::MultiBandBlender> blender_ =
			new cv::detail::MultiBandBlender(true, 5);
	std::vector<cv::UMat> channels_;
	cv::UMat stitchedFloat_;

	static void adjust_face_features(BeautyDemoPlan::Frames &frames,
			std::vector<cv::UMat> &channels,
			const BeautyDemoPlan::Params &params) {
		cv::UMat tmp;
		//boost saturation of eyes and lips
		adjust_saturation(frames.orig_, frames.eyesAndLips_,
				params.eyesAndLipsSaturation_, channels);
		//reduce skin contrast
		multiply(frames.orig_, cv::Scalar::all(params.skinContrast_),
				frames.skin_);
		//fix skin brightness
		add(frames.skin_,
				cv::Scalar::all((1.0 - params.skinContrast_) / 2.0) * 255.0,
				tmp);
		//boost skin saturation
		adjust_saturation(tmp, frames.skin_, params.skinSaturation_, channels);
	}

	static void stitch_face(cv::Ptr<cv::detail::MultiBandBlender> &bl,
			BeautyDemoPlan::Frames &frames, cv::UMat &stitchedFloat) {
		...
		bl->feed(frames.eyesAndLips_, frames.eyesAndLipsMaskGrey_,
				cv::Point(0, 0));
...
public:
	BeautyFilterPlan(const BeautyDemoPlan::Params &intputParams,
			BeautyDemoPlan::Frames &inputOutputFrames) :
			inputParams_(intputParams), inputOutputFrames_(inputOutputFrames) {
	}

	void infer() override {
		plain(adjust_face_features, RW(inputOutputFrames_), RW(channels_),
				CS(inputParams_))->plain(stitch_face, RW(blender_),
				RW(inputOutputFrames_), RW(stitchedFloat_));
	}
};
