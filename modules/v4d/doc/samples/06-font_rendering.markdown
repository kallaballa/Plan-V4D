...
public:
    void infer() override {
        // Render the text at the center of the screen. Note that you can load your own fonts.
...
The `V4DPlan` for this example is very simple. It stores the string we want to render as a member variable.
...
- **`text(…)`**: The function that actually draws the text to the screen at the given coordinates. The four-argument overload `(x, y, begin, end)` draws the half-open character range `[begin, end)`.
...
void setup() override {
    nvg([&]() {
        ...
        // The font files are staged in the build's assets/fonts/ directory.
