...
class VideoEditingPlan : public V4DPlan {
    ...
    // Property extends Edge which means it can be directly passed without an Edge-directive.
...
- **`runtime->setSource(src)`** and **`runtime->setSink(sink)`**: We attach these objects to the runtime. This makes them available to be used by special commands within a `V4DPlan`.
...
- **`capture()`**: This special function, available within a `V4DPlan`, interacts with the `Source` attached to the runtime. It decodes one frame from the input video and places it into the main framebuffer, making it ready for processing or rendering.
...
- They are attached to the V4D runtime to be used by a `V4DPlan`.
