...
class ShaderDemoPlan : public V4DPlan {
    ...
    // Static function to process mouse events. Returns true when the auto-zoom
    // animation should continue, false once the user has interacted.
...
public:
    ...
    // The setup() method initializes the scene and the camera state.
    void setup() override {
        // One-shot initialization of the camera.
    ...
    void infer() override {
        ...
        // Branch: if no user interaction, keep advancing the auto-zoom.
...
- **`MandelbrotScene`**: Just like `CubeScene` before, this class handles all the direct OpenGL calls: creating a quad to draw on, compiling the complex Mandelbrot fragment shader via `cv::v4d::init_shaders`, and providing a `render()` method that sets the shader's `uniform` variables before drawing.
...
This `V4DPlan` listens for mouse events to enable direct interaction.
