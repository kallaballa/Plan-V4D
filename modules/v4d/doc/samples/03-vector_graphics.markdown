...
public:
    void infer() override {
        ...
        nvg([](const Size &sz) {
            ...
            // Nvg calls work exactly like their c-function counterparts.
...
The structure is simple: a `V4DPlan` with all logic contained in the `infer()` method. Since there are no resources to load, the `setup()` method is not needed.
...
- **`clearScreen()`**: A helper function that calls `glClear(GL_COLOR_BUFFER_BIT)` at the start of each frame.
