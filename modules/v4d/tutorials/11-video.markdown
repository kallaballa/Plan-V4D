...
This example uses the same `cubescene.hpp` from the previous tutorial to manage the low-level OpenGL rendering. The main application logic is in `samples/video-demo.cpp`.

```cpp
// samples/video-demo.cpp
...
int main(int argc, char** argv) {
    if (argc != 2) {
        std::cerr << "Usage: video-demo <video-file>" << std::endl;
        exit(1);
    }
...
The `main` function and the `Plan`'s `setup()` and `teardown()` methods are very similar to our previous examples.
