...
This tutorial demonstrates how to load and display an image using Plan-V4D's NanoVG context. We will create a simple `Plan` that loads an image during its setup phase and then renders it to the screen in a continuous loop.

## The Code

Here is the complete source code for this example. You can find it in `samples/display_image_nvg.cpp`.
...
int main() {
    ...
    Plan::run<DisplayImageNVG>(7, samples::findFile("lena.jpg"));
}
```
...
We define a class `DisplayImageNVG` that inherits from `cv::v4d::Plan`. This class will encapsulate the logic for our image display task.
...
- **`Plan::run<DisplayImageNVG>(...)`**: This static method creates an instance of our `DisplayImageNVG` plan, passes the filename "lena.jpg" to its constructor, and starts the execution loop.

## Summary

In this tutorial, we've seen how to:
- Create a `Plan` to structure a graphical application.
